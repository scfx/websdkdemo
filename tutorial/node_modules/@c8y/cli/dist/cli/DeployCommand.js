...
class DeployCommand extends Commands_1.Commands {
    ...
    async pathZipToApp(path) {
        ...
        const resolveIfReady = () => {
            if (pendingFiles === 0 && closed) {
                const appDataCopy = lodash_1.cloneDeep(appData);
                const name = this.getName(appDataCopy, path_1.basename(path));
                const key = this.getKey(appDataCopy, name);
                const contextPath = this.getContextPath(appDataCopy, name);
                lodash_1.assign(appDataCopy, { name, key, contextPath });
                resolve(appDataCopy);
            }
        };
    ...
    async pathJsonToApp(path) {
        return {
            json: this.getAppData(await this.fileToAppData(path), path_1.dirname(path)),
    ...
    getAppData(fileData, path = '') {
        ...
        const name = this.getName(appData, path_1.basename(path));
        const key = this.getKey(appData, name);
        const contextPath = this.getContextPath(appData, name);
        lodash_1.assign(appData, { name, key, contextPath });
    ...
    async uploadApplication(application, buffer, filePath) {
        ...
        try {
            ...
            const { data } = await this.client.application.binary(application).upload(buffer, filePath);
            remoteData = data;
        }
        catch (err) {
            console.debug(err);
    ...
    getName(appData, fallbackStr = '') {
        let name = lodash_1.get(appData, 'name', fallbackStr);
    ...
    getKey(appData, name) {
        let key = lodash_1.get(appData, 'key');
        if (!key) {
            key = `${lodash_1.kebabCase(name)}-key`;
        }
        return key;
    }
    getContextPath(appData, name) {
        return lodash_1.get(appData, 'contextPath', name.toLowerCase());
    }
